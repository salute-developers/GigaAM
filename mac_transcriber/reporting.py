...
AI_DIRECT_SEGMENT_LIMIT = int(os.environ.get("MAC_TRANSCRIBER_AI_DIRECT_SEGMENT_LIMIT", "180"))
AI_CHUNK_SIZE = int(os.environ.get("MAC_TRANSCRIBER_AI_CHUNK_SIZE", "80"))
AI_REQUEST_ATTEMPTS = max(1, int(os.environ.get("MAC_TRANSCRIBER_AI_REQUEST_ATTEMPTS", "3")))
...
def build_local_report(
    *,
    ...
    decisions = _dedupe_report_items(_matching_items(
        transcript,
    ...
    action_items = _dedupe_action_items(_local_action_items(transcript))[:LOCAL_REPORT_ITEM_LIMIT]
    ...
    risks = _dedupe_report_items(_matching_items(
        transcript,
        title_prefix="Риск",
        patterns=[
            r"\bриск\b",
    ...
    coverage = build_coverage(transcript=transcript, adaptive_sections=adaptive_sections)
    ...
    *,
    ...
    try:
        return build_ai_report(
            ...
            report_model=report_model or DEFAULT_REPORT_MODEL,
            api_key=api_key,
...
def build_direct_ai_report(
    *,
                ...
                "content": json.dumps(
                    {
                        ...
                        "segments": [asdict(record) for record in base_report.transcript],
    ...
    try:
        ...
        raise ReportGenerationError("OpenAI response was not valid JSON") from exc

    report = _merge_ai_payload(base_report, ai_payload, report_model=report_model)
    try:
        validate_report_citations(report)
...
def build_chunked_ai_report(
    *,
    ...
    for index, records in enumerate(chunks, start=1):
        ...
        content = _extract_output_text(response)
        if not content:
            raise ReportGenerationError(f"AI chunk {index} did not contain output text")
        try:
            chunk_payload = json.loads(content)
        except json.JSONDecodeError as exc:
            raise ReportGenerationError(f"AI chunk {index} response was not valid JSON") from exc
        chunk_notes.append(_filter_payload_citations(chunk_payload, {record.segment_id for record in records}))
    ...
    try:
        ...
        return _merge_ai_chunk_notes_payload(
            ...
            chunk_notes,
            report_model=report_model,
            warning=f"AI synthesis fallback: {exc}",
        )
    ...
    report = _merge_ai_synthesis_payload(base_report, ai_payload, report_model=report_model)
    try:
        validate_report_citations(report)
    except ValueError as exc:
        raise ReportGenerationError(f"AI chunked report failed validation: {exc}") from exc
    return report
...
def _build_chunked_synthesis_payload(
    *,
    ...
    try:
        return _merge_ai_synthesis_payloads(base_report, batch_payloads)
...
def _request_ai_synthesis_payload(
    *,
        ...
        "input": [
            {
                ...
                "content": (
                    ...
                    "повторяющиеся пункты внутри входных заметок. Каждый пункт должен иметь "
    ...
    try:
        ...
        raise ReportGenerationError(f"{context} response was not valid JSON") from exc
...
def _merge_ai_synthesis_payloads(
    ...
    profile = next(
        (payload.get("profile") for payload in payloads if isinstance(payload.get("profile"), dict)),
        {
    ...
    action_items = _dedupe_payload_actions(_flatten_payload_list(payloads, "action_items"))[:20]
    open_questions = _dedupe_payload_items(_flatten_payload_list(payloads, "open_questions"))[:20]
    risks = _dedupe_payload_items(_flatten_payload_list(payloads, "risks"))[:20]
    notable_quotes = _dedupe_payload_items(_flatten_payload_list(payloads, "notable_quotes"))[:8]
    adaptive_sections = _dedupe_payload_sections(_flatten_payload_list(payloads, "adaptive_sections"))[:7]
    ...
    return {
        ...
        "adaptive_sections": adaptive_sections,
        "timeline": _dedupe_payload_timeline(_flatten_payload_list(payloads, "timeline"))[:14],
        "decisions": decisions,
        "action_items": action_items,
        "open_questions": open_questions,
        "risks": risks,
...
def write_report_artifacts(
    *,
    ...
    if make_pdf:
        try:
            pdf_path = render_report_pdf(html_path=html_path, pdf_path=output_dir / "report.pdf")
        except ReportGenerationError as exc:
            pdf_error = str(exc)
...
def transcript_records(segments: list[Any]) -> list[TranscriptRecord]:
    ...
    for index, segment in enumerate(ordered, start=1):
        ...
        records.append(
            TranscriptRecord(
                segment_id=str(_segment_value(segment, "segment_id", "") or f"S{index:04d}"),
    ...
    *,
    ...
    if profile.kind == "lecture":
        ...
        return [
            AdaptiveSection(
                ...
                summary=_shorten(" ".join(record.text for record in transcript), limit=900),
                items=notes,
    ...
    if profile.kind == "research_interview":
        insights = _matching_items(
            transcript,
            title_prefix="Инсайт",
            patterns=[r"\bпотому что\b", r"\bне удобно\b", r"\bболь\b", r"\bхочу\b", r"\bнужно\b"],
        )
        return [
            AdaptiveSection("interview_context", "Контекст респондента", "Понять ситуацию собеседника.", _shorten(" ".join(r.text for r in transcript[:4]), limit=700), _clone_report_items(notable_quotes[:3]), _unique_citations(notable_quotes[:3]), "blue"),
            AdaptiveSection("insights", "Инсайты", "Выделить наблюдения, которые полезны продукту.", "Повторяющиеся боли, мотивы и формулировки.", insights, _unique_citations(insights), "green"),
            AdaptiveSection("quotes", "Доказательные цитаты", "Сохранить голос пользователя.", "Цитаты для дальнейшего анализа.", _clone_report_items(notable_quotes), _unique_citations(notable_quotes), "violet"),
            AdaptiveSection("questions", "Открытые вопросы", "Что стоит уточнить в следующих интервью.", "Вопросы и неполные места.", _clone_report_items(open_questions), _unique_citations(open_questions), "amber"),
        ]

    if profile.kind == "technical_discussion":
        return [
            AdaptiveSection("architecture", "Архитектура и варианты", "Сохранить технические варианты и аргументы.", _shorten(" ".join(record.text for record in transcript), limit=900), _clone_report_items(notable_quotes), _unique_citations(notable_quotes), "blue"),
            AdaptiveSection("decisions", "Технические решения", "Отделить принятые решения от обсуждения.", "Принятые или предложенные технические решения.", _clone_report_items(decisions), _unique_citations(decisions), "green"),
            AdaptiveSection("risks", "Риски и блокеры", "Зафиксировать места, где возможна поломка.", "Технические риски, ограничения и неясности.", _clone_report_items(risks), _unique_citations(risks), "red"),
            AdaptiveSection("questions", "Открытые вопросы", "Что требует дополнительной проверки.", "Незакрытые технические вопросы.", _clone_report_items(open_questions), _unique_citations(open_questions), "amber"),
        ]
    ...
    if profile.kind == "consultation":
        recommendations = _matching_items(
            ...
            patterns=[r"\bрекоменд", r"\bлучше\b", r"\bстоит\b", r"\bнадо\b", r"\bнужно\b"],
        )
        return [
            AdaptiveSection("diagnosis", "Ситуация и диагноз", "Собрать проблему и контекст.", _shorten(" ".join(record.text for record in transcript), limit=800), _clone_report_items(notable_quotes[:4]), _unique_citations(notable_quotes[:4]), "blue"),
            AdaptiveSection("recommendations", "Рекомендации", "Сделать выводы применимыми.", "Рекомендованные действия и варианты.", recommendations, _unique_citations(recommendations), "green"),
            AdaptiveSection("actions", "План действий", "Превратить консультацию в next steps.", "Практические шаги после консультации.", _action_items_as_report_items(action_items), _unique_action_citations(action_items), "amber"),
        ]
...
def build_coverage(
    *,
    ...
    section_by_segment: dict[str, list[str]] = {record.segment_id: [] for record in transcript}
    ...
    for record in transcript:
        ...
        if titles:
            ...
            status = "low_signal"
            rationale = "Сегмент похож на короткую реплику, повтор или техническую связку."
        else:
            status = "supporting"
            rationale = "Сегмент сохранён в полном транскрипте и поддерживает общий контекст."
        entries.append(
            CoverageEntry(
...
def validate_report_citations(report: MeetingReport) -> None:
        ...
        for item in items:
            for segment_id in item.citations:
                if segment_id not in known:
                    raise ValueError(f"{section_name} references unknown segment id: {segment_id}")
    for section in report.adaptive_sections:
        for segment_id in section.citations:
            if segment_id not in known:
                raise ValueError(f"adaptive_sections references unknown segment id: {segment_id}")
        for item in section.items:
            for segment_id in item.citations:
                if segment_id not in known:
                    raise ValueError(f"adaptive_sections references unknown segment id: {segment_id}")
    for entry in report.coverage:
        if entry.segment_id not in known:
            raise ValueError(f"coverage references unknown segment id: {entry.segment_id}")
    ...
    duplicates = sorted({segment_id for segment_id in coverage_ids if coverage_ids.count(segment_id) > 1})
    if duplicates:
...
def render_report_markdown(report: MeetingReport) -> str:
    ...
    for block in data["timeline"]:
        lines.extend([f"### {block['range']} · {block['title']}", "", block["summary"], ""])
...
def build_protocol_data(report: MeetingReport) -> dict[str, Any]:
    ...
    participants = sorted({record.speaker for record in report.transcript if record.speaker})
        ...
        "coverage": [
            {
                ...
                "status": entry.status,
                "time": fmt_time(index[entry.segment_id].start) if entry.segment_id in index else "",
...
def render_slack_summary(
    *,
    ...
    date_parts = [str(meeting.get("date_human") or meeting.get("date_short") or "").strip()]
    ...
    participants = ", ".join(str(item) for item in protocol.get("participants", []) if item)
    kpis = {str(item.get("cap", "")).lower(): item.get("num") for item in protocol.get("kpis", [])}
    status = _slack_status_label(str(health.get("status") or "unknown"))
    alerts = _slack_health_alerts([str(item) for item in health.get("alerts", []) if item])
    ...
    for file_info in files:
        lines.append(f"• `{file_info['filename']}` — {file_info['title']} ({file_info['size_human']})")
...
def _slack_health_alerts(alerts: list[str]) -> list[str]:
    ...
    for alert in alerts:
        ...
        if has_openai_quota and message == "AI-обогащение не выполнено; использован локальный резервный режим.":
            continue
...
def _slack_artifact_files(*, output_dir: Path, pdf_path: Path | None) -> list[dict[str, Any]]:
    candidates = [
        ("report_pdf", pdf_path or output_dir / "report.pdf", "PDF-протокол", "application/pdf"),
        ("report_md", output_dir / "report.md", "Markdown-отчёт", "text/markdown"),
        ("transcript_md", output_dir / "transcript.md", "Markdown-транскрипт", "text/markdown"),
    ]
...
def _protocol_action_items(items: list[ActionItem], report: MeetingReport) -> list[dict[str, Any]]:
    rendered: list[dict[str, Any]] = []
    for item in items:
        ref = _protocol_ref(report, item.citations)
        if not ref:
...
def _protocol_tldr(report: MeetingReport) -> list[str]:
    points: list[str] = []
    for section in report.adaptive_sections:
        if section.kind in {"key_points", "architecture", "lecture_notes", "insights", "recommendations"}:
            for item in section.items:
        ...
        grouped.append(
            {
                ...
                "summary": _shorten(" ".join(item["summary"] for item in group), limit=760),
            }
        )
    return grouped[:8]
...
def _protocol_time_human(source_filename: str) -> str:
    ...
    if parsed is None:
        return ""
    match = re.search(r"\b(20\d{2})[-_\.](\d{2})[-_\.](\d{2})\s+(\d{2})[-_:](\d{2})\s*UTC\b", source_filename)
    if not match:
        return ""
...
def render_report_pdf(*, html_path: Path, pdf_path: Path) -> Path | None:
    ...
    playwright_bin = os.environ.get("MAC_TRANSCRIBER_PLAYWRIGHT") or shutil.which("playwright")
...
def _ensure_fontconfig_cache() -> None:
    if os.environ.get("XDG_CACHE_HOME"):
        return
    cache_dir = Path(os.environ.get("MAC_TRANSCRIBER_FONT_CACHE_DIR", "/tmp/mac-transcriber-font-cache"))
    try:
...
def build_report_health(
    *,
    ...
    def add_check(name: str, ok: bool, fail_detail: str, ok_detail: str | None = None) -> None:
    ...
    add_check(
        ...
        artifact_status["report_md"]["size_bytes"] >= (512 if report.segment_count else 1),
    ...
    protocol_ok, protocol_detail = _protocol_json_shape_status(artifact_files["report_json"])
    add_check(
        "protocol_json_shape",
        protocol_ok,
        protocol_detail,
    ...
    add_check(
        "report_has_no_warnings",
        not report.warnings,
        _report_warning_alert(report.warnings),
        "Report was generated without warnings.",
    )
    if requested_ai:
        add_check(
            "ai_report_succeeded",
            report.generated_by != "local" and not any("AI report fallback" in warning for warning in report.warnings),
    ...
    if requested_pdf:
        add_check(
            ...
            f"PDF generation failed: {pdf_error}" if pdf_error else "PDF report artifact is missing.",
...
def _protocol_json_shape_status(path: Path) -> tuple[bool, str]:
    ...
    required_top = {"meeting", "tldr", "decisions", "tasks", "questions", "risks", "timeline", "kpis"}
...
def _transcript_inventory_status(path: Path, *, expected_segments: int) -> tuple[bool, str]:
    if not path.exists():
    ...
    if explicit_ids == 0 and rows != expected_segments:
        return False, "Transcript Markdown merged rows without explicit raw S-ID ranges."
...
def _pdf_content_status(path: Path) -> tuple[bool, str]:
    ...
    for pattern in forbidden_patterns:
        if re.search(pattern, text):
            return False, "PDF contains embedded transcript rows or coverage audit text."
    ...
    return True, "PDF content gate OK: protocol text extracts, full transcript and coverage audit are not embedded."
...
def load_openai_api_key(env_path: Path | None = None) -> str | None:
    ...
    if key:
        return key.strip()
    configured = os.environ.get("MAC_TRANSCRIBER_OPENAI_ENV_FILE")
    candidates = [Path(configured)] if configured else []
    candidates.append(env_path or Path(".env.local"))
...
def _ai_adaptive_section_schema(item: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "required": ["kind", "title", "purpose", "summary", "items", "citations", "accent"],
        "additionalProperties": False,
    }
...
def _is_local_open_question(text: str) -> bool:
    ...
    if "?" not in clean:
        return False
    if re.search(r"\bчто\b.*\b(?:делать|идти|должно|дальше|после|будет|план)\b", lowered):
        return True
    if re.search(r"\bкак\b.*\b(?:проверять|отображать|попадать|согласовать|закрыть)\b", lowered):
        return True
...
def _chunks(records: list[TranscriptRecord], *, size: int) -> list[list[TranscriptRecord]]:
...
def _citation_text(report: MeetingReport, citations: list[str]) -> str:
    ...
    for segment_id in citations:
        ...
        if record is None:
            ...
            labels.append(f"{segment_id} {fmt_time(record.start)}-{fmt_time(record.end)}")
    return ", ".join(labels)
...
def _append_adaptive_sections_markdown(lines: list[str], report: MeetingReport) -> None:
    for section in report.adaptive_sections:
        ...
        if not section.items:
            ...
            continue
        for item in section.items:
            lines.append(f"- **{item.title}:** {item.text} ({_citation_text(report, item.citations)})")
        lines.append("")
...
def _append_coverage_markdown(lines: list[str], report: MeetingReport) -> None:
    ...
    lines.append("| --- | --- | --- | --- |")
    for entry in report.coverage:
        used_in = ", ".join(entry.section_titles) if entry.section_titles else "full transcript"
        lines.append(
...
def _append_timeline_markdown(lines: list[str], report: MeetingReport) -> None:
    ...
    if not report.timeline:
        ...
        return
    for block in report.timeline:
        lines.append(f"- **{block.title}:** {block.summary} ({_citation_text(report, block.citations)})")
    lines.append("")
...
def _post_openai_response(*, payload: dict[str, Any], api_key: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise ReportGenerationError(f"OpenAI API returned HTTP {exc.code}: {body}") from exc
...
def _post_openai_response_with_context(
    *,
    ...
    raise ReportGenerationError(f"{context} failed after {attempts_used} attempt(s): {detail}") from last_error
...
def _merge_ai_payload(
    ...
    *,
    ...
    profile = ReportProfile(
        ...
        confidence=float(profile_payload.get("confidence") or base_report.profile.confidence),
        rationale=str(profile_payload.get("rationale") or base_report.profile.rationale),
    )
    adaptive_sections = _adaptive_sections_from_payload(
        payload.get("adaptive_sections"),
        fallback=base_report.adaptive_sections,
    )
    coverage = _coverage_from_payload(payload.get("coverage"), fallback=base_report.coverage)
    return MeetingReport(
        ...
        decisions=_dedupe_report_items(_report_items(payload.get("decisions", []))),
        ...
        open_questions=_dedupe_report_items(_report_items(payload.get("open_questions", []))),
        risks=_dedupe_report_items(_report_items(payload.get("risks", []))),
        notable_quotes=_dedupe_report_items(_report_items(payload.get("notable_quotes", []))),
...
def _merge_ai_chunk_notes_payload(
    ...
    *,
    ...
    key_points = _dedupe_report_items(_chunk_report_items(chunk_notes, "key_points", title_prefix="Тема"))
    decisions = _dedupe_report_items(_chunk_report_items(chunk_notes, "decisions", title_prefix="Решение"))
    ...
    risks = _dedupe_report_items(_chunk_report_items(chunk_notes, "risks", title_prefix="Риск"))
    ...
    adaptive_sections = [
        AdaptiveSection(
            ...
            _shorten(" ".join(str(note.get("summary", "")) for note in chunk_notes), limit=1200),
            key_points,
    ...
    return MeetingReport(
        ...
        source_filename=base_report.source_filename,
        model_name=base_report.model_name,
        generated_by=f"{report_model}/chunked",
        segment_count=base_report.segment_count,
        duration=base_report.duration,
        overview=_shorten(" ".join(str(note.get("summary", "")) for note in chunk_notes), limit=1200),
...
def _flatten_payload_list(payloads: list[dict[str, Any]], key: str) -> list[dict[str, Any]]:
    items: list[dict[str, Any]] = []
...
def _dedupe_payload_sections(sections: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for section in sections:
        ...
        normalized = _dedupe_key(str(section.get("kind") or ""), str(section.get("title") or ""))
        existing = next(
            (
                candidate
                for candidate in merged
                if _dedupe_key(str(candidate.get("kind") or ""), str(candidate.get("title") or ""))
                == normalized
            ),
            None,
        )
        ...
        existing["citations"] = _unique_strings(
            ...
            + [citation for item in existing["items"] for citation in item.get("citations", [])]
        )
    return merged
...
def _dedupe_payload_timeline(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    merged: list[dict[str, Any]] = []
    for item in items:
        citations = _unique_strings(str(value) for value in item.get("citations", []) if value)
        if not citations:
            continue
        key = _dedupe_key(str(item.get("title") or ""), str(item.get("summary") or ""))
        if any(_dedupe_key(str(candidate.get("title") or ""), str(candidate.get("summary") or "")) == key for candidate in merged):
            continue
        copy = dict(item)
...
def _dedupe_payload_items(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for item in items:
        ...
        if existing is None:
            ...
            merged.append(copy)
            continue
        existing["citations"] = _unique_strings([*existing.get("citations", []), *citations])
...
def _items_are_duplicates(left_title: str, left_text: str, right_title: str, right_text: str) -> bool:
    ...
    for note in chunk_notes:
        for item in note.get(key, []):
            if not isinstance(item, dict):
                continue
            citations = [value for value in item.get("citations", []) if isinstance(value, str)]
            if not citations:
                continue
            items.append(
                ReportItem(
...
def _chunk_timeline(base_report: MeetingReport, chunk_notes: list[dict[str, Any]]) -> list[TimelineBlock]:
    ...
    for index, note in enumerate(chunk_notes, start=1):
        citations: list[str] = []
        for key in ["key_points", "decisions", "action_items", "open_questions", "risks", "notable_quotes"]:
            for item in note.get(key, []):
                if not isinstance(item, dict):
                    continue
                citations.extend(value for value in item.get("citations", []) if isinstance(value, str))
        citations = list(dict.fromkeys(citation for citation in citations if citation in record_by_id))
        if citations:
...
def _filter_payload_citations(payload: Any, known_ids: set[str]) -> Any:
    if isinstance(payload, dict):
        filtered = {key: _filter_payload_citations(value, known_ids) for key, value in payload.items()}
...
def _report_items(items: list[dict[str, Any]]) -> list[ReportItem]:
    return [
        ReportItem(
            title=str(item.get("title", "")),
            text=str(item.get("text", "")),
